...
export default function App({
  ...
  useEffect(() => {
    ...
    const initDynamicHitTest = async () => {
      unlistenFn = await listen<[number, number]>("neko_radar_tick", (event) => {
        ...
        if (el) {
          ...
          if (tag === 'canvas') {
            try {
              ...
              if (gl) {
                ...
                const px = Math.round((x - rect.left) * ((el as HTMLCanvasElement).width / rect.width));
                const py = Math.round((el as HTMLCanvasElement).height - ((y - rect.top) * ((el as HTMLCanvasElement).height / rect.height)));
          ...
          else if (el.id !== 'app' && el.id !== 'root' && tag !== 'body' && tag !== 'html') {
            shouldIgnore = false; 
          }
        }
        ...
        if (shouldIgnore !== isIgnoring) {
          ...
          invoke('set_click_through', { ignore: isIgnoring }).catch(console.error);
        }
      });
    };
